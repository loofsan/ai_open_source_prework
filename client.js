...
function prepareLabelForPlayer(player) {
  ...
  const w = temp.width;
  const h = temp.height;
  const radius = Math.floor(6 * dpr);
  const r = Math.min(radius, Math.floor(w / 2), Math.floor(h / 2));
  tctx2.moveTo(r, 0);
  tctx2.lineTo(w - r, 0);
  tctx2.quadraticCurveTo(w, 0, w, r);
  tctx2.lineTo(w, h - r);
  tctx2.quadraticCurveTo(w, h, w - r, h);
  tctx2.lineTo(r, h);
  tctx2.quadraticCurveTo(0, h, 0, h - r);
  tctx2.lineTo(0, r);
  tctx2.quadraticCurveTo(0, 0, r, 0);
  tctx2.closePath();
